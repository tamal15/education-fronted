...
const SyllbusTable = (props) => {
    
    const { driveLink, bookName, code, department, status, year, _id ,index,email} = props.data
    ...
    const handleUpdate = (id) => {
        fetch(`http://localhost:5000/updateSyllbus/${id}`, {
            method: "PUT",
        ...
        <>
            ...
            {/* <tbody key={_id}> */}
                 <tr role="row" style={{ question: "2px solid gray" }} >
                            ...
                            <td className='text-white'>{department}</td>
                            <td className='text-white'>{email}</td>
                            <td className='text-white'>{code}</td>
                            
                            <td> <iframe title="question" src={viewUrl}
                                className="img-fluid rounded-start w-100 " style={{ height: "110px" }} allow="autoplay"></iframe>
                            </td> 
